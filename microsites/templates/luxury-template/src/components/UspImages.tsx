...
function uspFromSelected(selected: any): { url: string }[] {
...
export function UspImages() {
  const selected = useSelectedCampaign()
  const items = uspFromSelected(selected)
  if (!items.length) return null

  const slides = items.map((it) => ({ src: it.url, alt: 'USP image' }))

  return (
    <section className="bg-cream px-6 py-16 min-[961px]:px-12 min-[961px]:py-20" id="usp">
      ...
      <Reveal effect="up" delay={60}>
        <h2 className="font-display mb-10 text-[clamp(1.8rem,3vw,2.6rem)] leading-tight font-normal text-dark">
          Project <em className="text-brown italic">Differentiators</em>
        </h2>
      </Reveal>

      <Reveal effect="up" delay={90}>
        <div className="relative overflow-hidden border border-border bg-beige shadow-[0_18px_54px_rgba(46,46,46,0.16)]">
